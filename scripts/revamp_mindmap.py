#!/usr/bin/env python3
"""
Revamp Data Engineering Mindmap — Immersive Full-Screen Collapsible Sidebars:
- Both Left Domains Sidebar and Right Inspector Panel are minimizable/collapsible.
- Sleek floating edge handles, top header toggle buttons, and keyboard shortcuts (⌘[, ⌘], ⌘F for Fullscreen).
- Ultra-slow, calm breathing floating simulation (viscous damping).
- Smooth, elegant Bezier curved paths (`Q cx,cy x2,y2`).
...
collapsible_template = """<!DOCTYPE html>
<html lang="en" class="theme-dark">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>DE.UNIVERSE — Data Engineering Knowledge Graph</title>
  <meta name="description" content="Editorial, data-dense interactive knowledge graph covering 250+ data engineering concepts with immersive collapsible sidebars and soothing organic curved motion.">
  ...
  <style>
    ...
    * {
    ...
    /* LEFT SIDEBAR — APPLE GLASS NAV (260px) WITH COLLAPSIBLE TRANSITION */
    .sidebar-left {
      width: 260px;
      background: var(--bg-elevated);
      backdrop-filter: var(--glass-blur);
      ...
      flex-shrink: 0;
      box-shadow: var(--glass-shadow);
      transition: transform 0.35s cubic-bezier(0.16, 1, 0.3, 1), margin-left 0.35s cubic-bezier(0.16, 1, 0.3, 1);
    }
    ...
    /* FLOATING EDGE TOGGLE DOCK BUTTONS */
    .edge-dock-btn {
      position: absolute;
      top: 50%;
    ...
    .edge-dock-btn.left-edge {
      left: 0;
      border-radius: 0 12px 12px 0;
      border-left: none;
    }

    .edge-dock-btn.right-edge {
      right: 0;
    ...
    /* RIGHT INSPECTOR — APPLE GLASS CARD (360px) WITH COLLAPSIBLE TRANSITION */
    .inspector-panel {
      width: 360px;
      background: var(--bg-elevated);
      backdrop-filter: var(--glass-blur);
      ...
      overflow-y: auto;
      box-shadow: var(--glass-shadow);
      transition: transform 0.35s cubic-bezier(0.16, 1, 0.3, 1), margin-right 0.35s cubic-bezier(0.16, 1, 0.3, 1);
    }
...
<body>
  ...
  <header class="header-bar">
    ...
    <div class="header-actions">
      <button class="btn-action" id="btn-toggle-left" title="Toggle Domains Sidebar (⌘[)">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6"/></svg>
        <span>Domains</span>
      </button>

      <button class="btn-action" id="btn-toggle-right" title="Toggle Inspector Panel (⌘])">
        <span>Inspector</span>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg>
      </button>

      <button class="btn-action" id="btn-toggle-fullscreen" title="Full Screen Immersive View (⌘F)">
  ...
  <main class="app-main">
    ...
    <aside class="sidebar-left" id="sidebar-left">
      <div class="sidebar-header">
        ...
        <button class="btn-close-ins" id="btn-close-sidebar-left" title="Collapse Sidebar">‹</button>
      </div>
    ...
    <section class="graph-viewport-container" id="graph-container">
      <!-- FLOATING EDGE TOGGLE DOCK BUTTONS -->
      <button class="edge-dock-btn left-edge" id="floating-toggle-left" title="Expand Domains Navigator (⌘[)">›</button>
      <button class="edge-dock-btn right-edge" id="floating-toggle-right" title="Expand Inspector Panel (⌘])">‹</button>
    ...
    <aside class="inspector-panel" id="inspector-panel">
      <div class="inspector-header">
        ...
        <div class="node-title-row">
          ...
          <button class="btn-close-ins" id="btn-close-inspector" title="Collapse Panel">✕</button>
        </div>
        <div class="badge-row">
  ...
  <footer class="footer-bar">
    ...
    <div>
      <span>Shortcuts: ⌘K (Search) · ⌘[ (Domains) · ⌘] (Inspector) · ⌘F (Fullscreen) · R (Reset)</span>
    </div>
  </footer>
  ...
  <script>
    ...
    // SIDEBAR & INSPECTOR COLLAPSIBLE TOGGLE LOGIC
    const sidebarLeft = document.getElementById('sidebar-left');
    const inspectorPanel = document.getElementById('inspector-panel');
    const floatingToggleLeft = document.getElementById('floating-toggle-left');
    const floatingToggleRight = document.getElementById('floating-toggle-right');
    const btnToggleLeft = document.getElementById('btn-toggle-left');
    const btnToggleRight = document.getElementById('btn-toggle-right');
    const btnToggleFullscreen = document.getElementById('btn-toggle-fullscreen');

    function toggleLeftSidebar(forceState) {
      const isCollapsed = forceState !== undefined ? forceState : !sidebarLeft.classList.contains('collapsed');
      sidebarLeft.classList.toggle('collapsed', isCollapsed);
      btnToggleLeft.classList.toggle('active', !isCollapsed);
      floatingToggleLeft.style.display = isCollapsed ? 'flex' : 'none';

      setTimeout(resetGraphView, 350);
    }
    ...
    function toggleRightPanel(forceState) {
      ...
      floatingToggleRight.style.display = isCollapsed ? 'flex' : 'none';

      // Update zoom toolbar position
      const zoomToolbar = document.querySelector('.zoom-toolbar');
      if (zoomToolbar) zoomToolbar.style.right = isCollapsed ? '24px' : '384px';

      setTimeout(resetGraphView, 350);
    }

    function toggleFullscreenMode() {
      const bothCollapsed = sidebarLeft.classList.contains('collapsed') && inspectorPanel.classList.contains('collapsed');
      if (bothCollapsed) {
    ...
    document.getElementById('btn-close-sidebar-left').addEventListener('click', () => toggleLeftSidebar(true));
    document.getElementById('btn-close-inspector').addEventListener('click', () => toggleRightPanel(true));
    floatingToggleLeft.addEventListener('click', () => toggleLeftSidebar(false));
    floatingToggleRight.addEventListener('click', () => toggleRightPanel(false));
    ...
    function selectNode(id) {
      ...
      // Auto expand inspector if collapsed on node click
...
print("Successfully applied collapsible/minimizable vertical sidebars to mindmap!")
